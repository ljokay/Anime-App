...
const Compare = () => {
    ...
    const fetchMAL = async (username) => {
        try {
            let allAnime = [];
            let nextPage = `https://api.myanimelist.net/v2/users/${username}/animelist?status=completed&fields=list_status`;
            ...
            while (nextPage) {
                ...
                nextPage = result.paging?.next ? `${result.paging.next}` : null;
